...
class GeneralMultiPLE(Task):
    ...
    #     # Load from local directory if exists, otherwise from HuggingFace
    #     local_path = f"/app/MultiPL-E/humaneval-{language}"
    ...
    def __init__(self, language):
        ...
        if os.path.exists(local_path):
            ...
            self.dataset = load_dataset("parquet", data_files={"test": parquet_files})
        else:
            self.dataset = load_dataset(
                GeneralMultiPLE.DATASET_PATH,
                self.DATASET_NAME,
