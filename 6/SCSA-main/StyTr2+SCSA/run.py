...
for i in range(0, len(data_folders)):
    ...
    c_name = cnt.split('/')[-1].split('.')[0]
    s_name = sty.split('/')[-1].split('.')[0]
    sem_map_32 = '../sem_precomputed_feats/' + number + '/' + s_name + '_' + c_name + '_map_32.pt'
    sem_map_64 = '../sem_precomputed_feats/' + number + '/' + s_name + '_' + c_name + '_map_64.pt'
    ...
    result = subprocess.run(['python', 'StyTr2.py', '--content', cnt, '--style', sty],
                capture_output=True,text=True)


    result = subprocess.run(['python', 'StyTr2.py', '--content', sty, '--style', cnt],
                capture_output=True,text=True)
