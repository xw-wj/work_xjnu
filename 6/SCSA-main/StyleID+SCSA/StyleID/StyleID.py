...
def feat_merge(opt, cnt_feats, sty_feats, start_step):
    feat_maps = [{'config': {
                'T':opt.T,
                'gamma':opt.gamma,
                'timestep':_,
...
def main():
    ...
    parser.add_argument('--output_path', type=str, default='output_StyleID')
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                output_name = opt.cnt.split('/')[-1].split('.')[0] + '_' + opt.sty.split('/')[-1].split('.')[0] + "_StyleID" + ".jpg"

                
                adain_z_enc = adain(cnt_z_enc, sty_z_enc)
                feat_maps = feat_merge(opt, cnt_feat, sty_feat, start_step)
